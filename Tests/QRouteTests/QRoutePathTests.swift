...
class QRoutePathTests: XCTestCase {
    ...
    func testFindPath_two() {
        given("2 routes: route with child route") {
            let child = QRoute("island")
            let parent = QRoute("archipelago", child)
    ...
    func testFindPath_many() {
        given("route plan with many nested routes (MockQRoutePlan)") {
            ...
            when("'ZachTwo' tries to find lowest common ancestor for invalid route") {
                let foreignRoute = QRoute("La La Land")
    ...
    func testHashable() {
        given("path from parent to child") {
            let child = QRoute("child")
            let parent = QRoute("parent", child)
            let path = parent.findPath(to: child.id)
            when("converted to a set") {
                let result = Set<QRoutePathNode>( path )
